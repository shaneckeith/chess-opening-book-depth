"""
extract_bands_full.py

Full-population version of extract_bands_test.py. Streams a .zst PGN
export. For each game, reads the raw text block (headers + movetext)
into memory as plain strings first - no python-chess involved. Checks
rapid/band/termination via simple string search on the header lines.
Every game that passes ALL cheap filters gets parsed by chess.pgn (via
io.StringIO on the buffered text) for book-depth scoring - no sampling
gate, every eligible game is kept.
...
def process_zst(zst_path):
    print(f"Opening {zst_path} ...", flush=True)
    band_files = {name: open(f"full_{name}.csv", "w", newline="", encoding="utf-8") for name, _, _ in BANDS}
    ...
    for f in band_files.values():
        f.close()
    return total_games, kept_games, band_counts, time.time() - start


if __name__ == "__main__":
    infile = sys.argv[1] if len(sys.argv) > 1 else "lichess_db_standard_rated_2023-06.pgn.zst"
    print("=== Process started: band extraction (full population, no sampling) ===", flush=True)
    print(f"Input file: {infile}", flush=True)
    total, kept, counts, elapsed = process_zst(infile)

    summary_lines = []
    summary_lines.append("=== DONE ===")
    summary_lines.append(f"Total games scanned: {total:,}")
    summary_lines.append(f"Total games kept:    {kept:,}")
    for band_name, _, _ in BANDS:
        summary_lines.append(f"  {band_name}: {counts[band_name]:,} games -> full_{band_name}.csv")
    summary_lines.append(f"Runtime: {elapsed:.0f}s ({elapsed/3600:.2f}h)")
    rate = total / elapsed if elapsed > 0 else 0
    summary_lines.append(f"Rate: {rate:.0f} games/sec")

    summary_text = "\n".join(summary_lines)
    print("\n" + summary_text)

    with open("extract_full_summary.txt", "w", encoding="utf-8") as f:
        f.write(summary_text + "\n")
    print("\n(summary also saved to extract_full_summary.txt)")
